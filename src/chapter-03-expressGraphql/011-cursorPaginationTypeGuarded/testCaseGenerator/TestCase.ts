...
class TestCase {
    ...
    constructor(userSize: number, postSize: number) {
        if (userSize < 0 || LoLChampions.length < userSize) throw new Error(`userSize is ${0}~${LoLChampions.length}`);
        if (postSize < 0 || LoremIpsum.length < postSize) throw new Error(`postSize is ${0}~${LoLChampions.length}`);
        this.userList = [];
        this.postList = [];
        ...
        shuffle<string>(LoremIpsum)
            ...
            .forEach((title, idx) => {
                ...
                let likeThisPostCnt = Math.floor(Math.random() * 1000) % userSize;
                let likeList: Like[] = [];
                shuffle<User>(this.userList)
                    ...
                    .forEach((user) => {
                        ...
                        likeList.push({
                            who: user,
                            when: Math.floor(Math.random() * (max - min + 1) + min)
                        });
                    });
                this.postList.push({
                    title: title,
