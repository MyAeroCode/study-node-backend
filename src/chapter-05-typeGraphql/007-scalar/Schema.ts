...
import { buildSchemaSync, Query, Arg, Int } from "type-graphql";
 ...
 * 외부적 표현 : hex string
 */
let RGBSclar = new GraphQLScalarType({
    name: "RGB",
    /**
...
/**
 * 타입 스크립트에 사용될 스칼라 타입 객체.
 */
class RGB {
    private innerData: number[];
    constructor(input: string | number[]) {
        if (typeof input == "string") {
            this.innerData = str2arr(input);
        } else {
    ...
    toStr(): string {
        return arr2str(this.innerData);
    }
}

/**
 * 반환형에 GraphQLScalarType 객체를 기입하고,
 * 스키마의 scalarsMap에 {대응되는 클래스 이름, GraphQLScalarType 객체}를 넣는다.
 */
class RootSchema {
    @Query((type) => RGBSclar)
    arr2rgb(@Arg("arr", (type) => [Int]) arr: number[]) {
        return arr;
    }

    @Query((type) => [Int])
    rgb2arr(@Arg("rgb", (type) => RGBSclar) arr: Int8Array) {
        return arr;
    }
}
export const schema = buildSchemaSync({
    resolvers: [RootSchema],
    scalarsMap: [{ type: RGB, scalar: RGBSclar }]
});
